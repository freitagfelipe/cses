...
public:
    ...
    T query(int l, int r) {
        return max(this->query(1, 1, this->n, l, r).max_prefix_sum, (T) 0);
    }
...
private:
    struct Node {
        ...
        Node operator+(const Node &o) const {
            ...
            node.all_prefix_sum = this->all_prefix_sum + o.all_prefix_sum;

            node.max_prefix_sum = max(this->max_prefix_sum, this->all_prefix_sum + o.max_prefix_sum);
    ...
    void build(U *v, int node, int l, int r) {
        if (l == r) {
            this->arr[l] = v[l - 1];
            this->tree[node].max_prefix_sum = v[l - 1];
            this->tree[node].all_prefix_sum = v[l - 1];
    ...
    void update(int node, int l, int r, int target, U v) {
        if (l == r) {
            this->arr[l] = v;
            this->tree[node].max_prefix_sum = v;
            this->tree[node].all_prefix_sum = v;
